...
const ReserveCard = () => {
  ...
  return (
    <div className="bg-white rounded-xl flex flex-col  relative w-full text-black  max-w-xl p-6    ">
      ...
      { show ?( 
        <>
         <h4 className="mt-4 text-xl sm:text-3xl font-serif">Reserve 
         <span className="font-semibold"> Noovo  
            </span></h4>
            <hr  className="mt-12 text-gray-300"/>
          <div className="absolute right-4 top-4 ">
            {/* <button onClick={()=>setShow(!show)} className="bg-gray-200 font-semibold rounded-3xl px-3 py-1">X</button> */}
          </div>
          <div className="pt-8 space-y-6 w-full">
            ...
            <hr  className=" md:mt-12 text-gray-300"/>
          </div>
      
          <div className=" pt-4 md:pt-8">
            <p className="text-[14px]">Transparent Pricing</p>
            <h4 className="text-xl md:text-2xl font-semibold">Starting at: $162,700.</h4>
          </div>
         ...
         <button onClick={()=>setShow(!show)} className="bg-[#F3B03D] rounded-4xl w-full g py-4"> Reserve Your Noovo Lite</button>
         </div>
